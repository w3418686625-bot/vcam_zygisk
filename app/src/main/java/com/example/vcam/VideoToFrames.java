...
public class VideoToFrames implements Runnable {
    ...
    public void videoDecode(String videoFilePath) throws IOException {
        ...
        try {
            ...
            if (trackIndex < 0) {
                Log.i(TAG,"【VCAM】【decoder】No video track found in " + videoFilePath);
            }
            ...
            if (isColorFormatSupported(decodeColorFormat, decoder.getCodecInfo().getCapabilitiesForType(mime))) {
                ...
                Log.i(TAG,"【VCAM】【decoder】set decode color format to type " + decodeColorFormat);
            } else {
                Log.i(TAG, "unable to set decode color format, color format type " + decodeColorFormat + " not supported");
                Log.i(TAG,"【VCAM】【decoder】unable to set decode color format, color format type " + decodeColorFormat + " not supported");
            }
            ...
            Log.i(TAG,"【VCAM】[videofile]"+ e.toString());
        } finally {
            if (decoder != null) {
                decoder.stop();
                decoder.release();
                    ...
                    if (play_surf == null) {
                        ...
                        if (mQueue != null) {
                            try {
                                ...
                                Log.i(TAG,"【VCAM】" + e.toString());
                            }
                        }
                        if (outputImageFormat != null) {
                            // 帧数据在此处理（系统级方案中由 native hook 使用）
                        }
                        image.close();
                    }
